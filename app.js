...
const filteredInventors = inventors.filter((inventor) => {
   oldInventors = inventor.year >1499 && inventor.year < 1600
   return oldInventors
});
veryOldInventors = filteredInventors
// Check your work:
...
// Complete the exercise in the space below:
const formatNames = inventors.map((inventor) => {
    fName = inventor.first
    lName = inventor.last
    return { first : fName, last: lName }
})

inventorNames = formatNames
// Check your work:
...
const sortInventors = inventors.sort((a, b) => {
 sortThis = a.year - b.year
 return sortThis
})
sortedByBirthYear = sortInventors
// Check your work:
...
const namesOnly = people.map((pep) => {
    names = pep.split(',')
...
const isAdult = devs.some((dev) => {
    age = new Date().getFullYear() - dev.year
    return age > 17
})
...
const isNineteen = devs.every((nine) => {
    ageNineteen = new Date().getFullYear() - nine.year
    return ageNineteen > 18
    
})

isEveryone19OrOlder = isNineteen
